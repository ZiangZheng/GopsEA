...
import numpy as np
import importlib
import gops_tasks.original.create_pkg.create_env_model as ce
from gops_tasks.original.env.inspector.env_data_checker import check_env_file_structures
        ...
        for sub in ["env_archive", "env_gym", "env_matlab", "env_ocp"]:
            try:
                file_obj = importlib.import_module(
                    "gops_tasks.original.env." + sub + "." + env_file_name
                )
                break
            except:
                pass
