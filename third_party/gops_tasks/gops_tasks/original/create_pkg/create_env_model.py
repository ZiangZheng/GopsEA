...
import numpy as np
from gops_tasks.original.env.wrapper.action_repeat import ActionRepeatModel
from gops_tasks.original.env.wrapper.clip_action import ClipActionModel
from gops_tasks.original.env.wrapper.clip_observation import ClipObservationModel
from gops_tasks.original.env.wrapper.mask_at_done import MaskAtDoneModel
from gops_tasks.original.env.wrapper.scale_action import ScaleActionModel
from gops_tasks.original.env.wrapper.scale_observation import ScaleObservationModel
from gops_tasks.original.env.wrapper.shaping_reward import ShapingRewardModel
...
for env_dir_name in env_dir_list:
    ...
    for file in file_list:
        if file.endswith(".py") and file[0] != "_" and "base" not in file:
            env_id = file[:-3]
            mdl = importlib.import_module(f"gops_tasks.original.env.{env_dir_name}.env_model.{env_id}")
