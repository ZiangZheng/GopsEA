...
from RenforceRL.components.critic import MultiQNetwork
...
class SAC(AlgorithmBase):
    ...
    def __init__(
        ...
        self.critic_optimizer = optim.Adam(
            self.critics.parameters(), lr=cfg.critic_lr, betas=(0.9, 0.999)
        )
    ...
    def update(self, generator: Generator[Dict[str, torch.Tensor], None, None]):
        ...
        for minib in generator:
            ...
            critic_loss, q_mean, target_q_mean, actor_loss, alpha_loss, alpha, entropy = self.update_seq(
                **params_dict,
            )
    ...
    def update_critic(
        ...
        # Bootstrap mask: only mask out on termination (not timeout)
        # Termination: episode truly ended (task done/failed) -> no bootstrap
        # Timeout: episode ended due to time limit -> should bootstrap
