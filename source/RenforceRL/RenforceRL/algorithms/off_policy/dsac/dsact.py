...
class DSACT(AlgorithmBase):
    ...
    def __init__(
        ...
        self.critic1_optimizer = optim.Adam(
            self.critic1.parameters(), lr=cfg.critic_lr, betas=(0.9, 0.999)
        )
        self.critic2_optimizer = optim.Adam(
            self.critic2.parameters(), lr=cfg.critic_lr, betas=(0.9, 0.999)
        )
    ...
    def update(self, generator: Generator[Union[PipeBufferTransition.TransitionData, dict], None, None]):
        self.ptr_update = 0
        critic_losses, avg_q1_mean_s, avg_q2_mean_s, avg_q1_std_s, avg_q2_std_s, actor_losses, alpha_losses, alphas, entropies, log_prob_means = [], [], [], [], [], [], [], [], [], []
        for minib in generator:
            if isinstance(minib, dict):
                critic_loss, avg_q1_mean, avg_q2_mean, avg_q1_std, avg_q2_std, min_q1_std, min_q2_std, actor_loss, alpha_loss, alpha, entropy, log_prob_mean = self.compute_gradient(**minib)
                self.update_trans()
            else:
        ...
        return {
            ...
            "log_prob_mean": sum(log_prob_means) / len(log_prob_means),
    ...
    def compute_gradient(
        ...
        critic_loss, avg_q1_mean, avg_q2_mean, avg_q1_std, avg_q2_std, min_q1_std, min_q2_std = self.compute_loss_critic(obs, critic_obs, actions, rewards, next_obs, next_critic_obs, termination, timeout)
        ...
        if self.auto_entropy:
            ...
            alpha_loss, log_prob_mean = self.compute_loss_alpha(obs)
            alpha_loss.backward()
        else:
            alpha_loss = None

        return critic_loss, avg_q1_mean, avg_q2_mean, avg_q1_std, avg_q2_std, min_q1_std, min_q2_std, actor_loss, alpha_loss, self._get_alpha(), entropy, log_prob_mean
    ...
    def _compute_target_q(self, r, done, q_mean, q_std, q_mean_next, q_sample_next, log_prob_next):
        target_q = r + (1 - done) * self.cfg.gamma * (
            q_mean_next - self._get_alpha() * log_prob_next
        )
        target_q_sample = r + (1 - done) * self.cfg.gamma * (
            q_sample_next - self._get_alpha() * log_prob_next
        )
        td_bound = 3 * q_std
    ...
    def compute_loss_critic(
        ...
        q1_mean_next, _, q1_sample_next = self._q_evaluate(
            next_critic_obs, next_actions, self.target_critic1, use_min=False
        )
        q2_mean_next, _, q2_sample_next = self._q_evaluate(
            next_critic_obs, next_actions, self.target_critic2, use_min=False)
        q_mean_next = torch.min(q1_mean_next, q2_mean_next)
        # q_mean_next = 0.5 * (q1_mean_next + q2_mean_next)
        ...
        critic1_loss = torch.mean(ratio1 *(huber_loss(q1_mean, target_q1, delta = 50, reduction='none') 
                                      + q1_std *(q1_std_detach.pow(2) - huber_loss(q1_mean.detach(), target_q1_bound, delta = 50, reduction='none'))/(q1_std_detach +bias)
                            ))
        critic2_loss = torch.mean(ratio2 *(huber_loss(q2_mean, target_q2, delta = 50, reduction='none')
                                      + q2_std *(q2_std_detach.pow(2) - huber_loss(q2_mean.detach(), target_q2_bound, delta = 50, reduction='none'))/(q2_std_detach +bias)
                            ))
        critic_loss = critic1_loss + critic2_loss
        
        return critic_loss, float(q1_mean.detach().mean()), float(q2_mean.detach().mean()), float(q1_std.detach().mean()), float(q2_std.detach().mean()), float(q1_std.min().detach()), float(q2_std.min().detach())
    ...
    def compute_loss_actor(
        ...
        # actor_loss = (self._get_alpha() * log_prob - 0.5 * (q1_mean + q2_mean)).mean()
    ...
    def compute_loss_alpha(
        ...
        return 0.0 if alpha_loss is None else alpha_loss, float(log_prob.mean().item())
...
class DSACTCfg(AlgorithmBaseCfg):
    ...
    def construct_from_cfg(self, actor_critic, device, *args, **kwargs):
        return DSACT(self, actor=actor_critic.actor, critic1=actor_critic.critic[0], critic2=actor_critic.critic[1], device=device, *args, **kwargs)