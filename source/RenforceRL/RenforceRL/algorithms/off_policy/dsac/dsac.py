...
class DSAC(AlgorithmBase):
    ...
    def update(self, generator: Generator[Union[PipeBufferTransition.TransitionData, dict], None, None]):
        ...
        for minib in generator:
            if isinstance(minib, dict):
                critic_loss, q_mean_mean, target_q_mean, actor_loss, alpha_loss, alpha, entropy = self.compute_gradient(**minib)
                self.update_trans()
            else:
    ...
    def compute_gradient(
        ...
        critic_loss, q_mean_mean, target_q_mean \
            = self.compute_loss_critic(obs, critic_obs, actions, rewards, next_obs, next_critic_obs, termination, timeout)
    ...
    def _compute_target_q(self, r, done, q_mean, q_std, q_next_sample, log_prob_next):
        target_q = r + (1 - done) * self.cfg.gamma * (
            q_next_sample - self._get_alpha() * log_prob_next
        )
    ...
    def compute_loss_alpha(
        ...
        return 0.0 if alpha_loss is None else float(alpha_loss.item())
