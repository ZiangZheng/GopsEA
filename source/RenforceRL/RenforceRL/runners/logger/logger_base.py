...
class LoggerBase:
    ...
    def _log_statistics_string(self, runner, locs, pad):
        ...
        if self.logger_type != "wandb":
            # TensorBoard/W&B/Neptune 的 global_step 本质上是整型 step。
            # 这里原先直接用浮点秒 `self.tot_time` 会发生截断/重复 step，
            # 在长时间训练或断点续训时容易导致曲线前段看起来“消失/被覆盖”。
            time_step_ms = int(self.tot_time * 1000)
            self.writer.add_scalar(
                "Train/mean_reward/time", mean_rew, time_step_ms
            )
            self.writer.add_scalar(
                "Train/mean_episode_length/time", mean_len, time_step_ms
            )
...
class LoggerBaseCfg(ModuleBaseCfg):
    ...
    neptune_project : str = "RenforceRL" # """The neptune project name. Default is "RenforceRL"."""
    wandb_project   : str = "RenforceRL" # """The wandb project name. Default is "RenforceRL"."""